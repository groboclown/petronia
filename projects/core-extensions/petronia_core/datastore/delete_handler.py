...
def register_delete_data_listener(context: EventRegistryContext) -> StdRet[None]:
    ...
    return join_none_results(
        send_register_listeners(
            ...
            (
                (datastore_event.DeleteDataEvent.FULL_EVENT_NAME, None),
            ),
        ),
        context.register_event_parser(
            datastore_event.DeleteDataEvent.FULL_EVENT_NAME,
            datastore_event.DeleteDataEvent.parse_data,
        ),
...
class DeleteDataHandler(ContextEventObjectTarget[datastore_event.DeleteDataEvent]):
    """Handles the store data requests."""

    def on_context_event(
            self, context: EventRegistryContext, source: str, target: str,
            event: datastore_event.DeleteDataEvent,
    ) -> bool:
