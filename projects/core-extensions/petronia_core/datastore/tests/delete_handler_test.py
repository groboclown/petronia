...
from .. import delete_handler, shared_state
...
class DeleteHandlerTest(unittest.TestCase):
    ...
    def test_register_delete_data_listener__errors(self) -> None:
        ...
        res = delete_handler.register_delete_data_listener(context)
    ...
    def test_delete_event__does_not_exist(self) -> None:
        ...
        handler = delete_handler.DeleteDataHandler(context)
        handler.on_event(
            's1', 't', datastore.DeleteDataEvent(),
        )
    ...
    def test_delete_event__exists(self) -> None:
        ...
        shared_state.clear_data()
        shared_state.store_data('s:1', 'my-data')
        shared_state.store_data('s:2', 'my-data-2')
        handler = delete_handler.DeleteDataHandler(context)
        handler.on_event(
            's:1', 't', datastore.DeleteDataEvent(),
        )
