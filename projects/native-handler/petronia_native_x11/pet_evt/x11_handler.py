...
class X11NativeHandler(window.AbstractWindowHandler[X11NativeWindow, xcb_native.XcbWindow]):
    ...
    def register_messages(self, loop: message_loop.WindowsMessageLoop) -> None:
        ...
        self.__queue.add_handler(_CLOSE_WINDOW, X11NativeHandler._queue_close_window)
        self.__queue.add_handler(_MINIMIZE_WINDOW, X11NativeHandler._queue_minimize_window)
        self.__queue.add_handler(_MAXIMIZE_WINDOW, X11NativeHandler._queue_maximize_window)
        self.__queue.add_handler(_RESTORE_WINDOW, X11NativeHandler._queue_restore_window)
        self.__queue.add_handler(
            _SET_POSITION, lambda x: self._queue_set_position_window(self.__screen_mapper, x),
        )
        self.__queue.add_handler(_SET_STYLE, X11NativeHandler._queue_set_window_style)
    ...
    def on_set_window_position_request(  # pylint:disable=arguments-differ
            self, wnd: X11NativeWindow, new_location: window_events.ScreenDimension,
    ) -> StdRet[None]:
    ...
    def on_set_window_settings(  # pylint:disable=arguments-differ
            self, wnd: X11NativeWindow, settings: Mapping[str, str],
    ) -> StdRet[None]:
        if self.__queue:
            return self.__queue.queue_message(
    ...
    def on_set_focused_window(self, wnd: X11NativeWindow, focus_id: int) -> StdRet[None]:
        if self.__queue:
            return self.__queue.queue_message(
                _FOCUS_WINDOW, wnd,
            )
    ...
    def _in_exec_update_window_states(self, window_states: List[X11NativeWindow]) -> None:
    ...
    def _in_exec_update_window_state(self, window_state: X11NativeWindow) -> None:
        log.debug(
    ...
    def _in_exec_create_window_state(self, window_state: X11NativeWindow) -> None:
        log.debug(
            "Sending window created event: {window_id}",
            window_id=window_state.window_id,
        )
...
def is_manageable(wnd: X11NativeWindow) -> bool:
