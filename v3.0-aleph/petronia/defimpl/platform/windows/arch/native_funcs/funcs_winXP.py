...
def process__get_executable_filename(thread_pid: DWORD) -> Union[str, WindowsErrorMessage, None]:
        ...
        if pid == thread_pid:
            ...
            try:
                ...
                res = EnumProcessModules(
                    hproc.value, hmod_list, DWORD(c_sizeof(hmod_list)), byref(count_allocated_space)
                )
                if res == 0:
                    # Error; ignore
                    continue
                for j in range(count_allocated_space.value // c_sizeof(HMODULE)):
                    ...
                    res = GetModuleBaseName(hproc, hmod_list[j], mod_name, windows_constants.MAX_FILENAME_LENGTH + 1)
                    if res != 0:
...
def process__get_username_domain_for_pid(thread_pid: DWORD) -> Union[Tuple[str, str], WindowsErrorMessage]:
    ...
    hproc = windll.kernel32.OpenProcess(windows_constants.PROCESS_QUERY_INFORMATION, False, thread_pid)
    if hproc == 0:
    ...
    try:
        ...
        try:
            ...
            res = GetTokenInformation(
                access_token, windows_constants.TOKEN_INFORMATION__TOKEN_USER, None, 0, byref(length)
            )
            if res == 0:
            ...
            res = GetTokenInformation(
                access_token, windows_constants.TOKEN_INFORMATION__TOKEN_USER, sid_info, length, byref(length)
            )
            if res == 0:
...
def process__get_all_service_information() -> Union[WindowsErrorMessage, Sequence[ProcessMetrics]]:
    ...
    try:
        ...
        for i in range(service_count.value):
            ...
            controls_accepted = []
            if _has_bit_set(status.dwControlsAccepted, windows_constants.SERVICE_ACCEPT_NETBINDCHANGE):
                controls_accepted.append('SERVICE_ACCEPT_NETBINDCHANGE')
            if _has_bit_set(status.dwControlsAccepted, windows_constants.SERVICE_ACCEPT_PARAMCHANGE):
                controls_accepted.append('SERVICE_ACCEPT_PARAMCHANGE')
            if _has_bit_set(status.dwControlsAccepted, windows_constants.SERVICE_ACCEPT_PAUSE_CONTINUE):
                controls_accepted.append('SERVICE_ACCEPT_PAUSE_CONTINUE')
            if _has_bit_set(status.dwControlsAccepted, windows_constants.SERVICE_ACCEPT_PRESHUTDOWN):
...
def shell__get_raw_window_metrics() -> Union[NONCLIENTMETRICS, WindowsErrorMessage]:
    ...
    res = SystemParametersInfoW(windows_constants.SPI_GETNONCLIENTMETRICS, metrics.cbSize, byref(metrics), 0)
    if res != 0:
...
def inner__set_window_metrics(metrics: Union[NONCLIENTMETRICS, WindowMetrics]) -> Optional[WindowsErrorMessage]:
    ...
    if isinstance(metrics, WindowMetrics):
        ...
        m.iBorderWidth = DWORD(metrics.border_width)
        m.iScrollWidth = DWORD(metrics.scroll_width)
        m.iScrollHeight = DWORD(metrics.scroll_height)
        m.iCaptionWidth = DWORD(metrics.caption_width)
        m.iCaptionHeight = DWORD(metrics.caption_height)
        _dict_to_font(metrics.caption_font, m.lfCaptionFont)
        m.iSmCaptionWidth = DWORD(metrics.sm_caption_width)
        m.iSmCaptionHeight = DWORD(metrics.sm_caption_height)
        _dict_to_font(metrics.sm_caption_font, m.lfSmCaptionFont)
        m.iMenuWidth = DWORD(metrics.menu_width)
        m.iMenuHeight = DWORD(metrics.menu_height)
...
def _dict_to_font(f: FontMetrics, ret: Optional[LOGFONT] = None) -> LOGFONT:
    if ret is None:
        ret = LOGFONT()
    ret.lfHeight = LONG(f.height)
    ret.lfWidth = LONG(f.width)
    ret.lfEscapement = LONG(f.escapement)
    ret.lfOrientation = LONG(f.orientation)
    ret.lfWeight = LONG(f.weight)
    ret.lfItalic = BYTE(f.italic)
    ret.lfUnderline = BYTE(f.underline)
    ret.lfStrikeOut = BYTE(f.strikeout)
    ret.lfCharSet = BYTE(f.charset)
    ret.lfOutPrecision = BYTE(f.out_precision)
    ret.lfClipPrecision = BYTE(f.clip_precision)
    ret.lfQuality = BYTE(f.quality)
    ret.lfPitchAndFamily = BYTE(f.pitch_and_family)
    if f.face_name:
        ret.lfFaceName = f.face_name[:windows_constants.LF_FACESIZE - 1]
    return ret
...
def shell__set_border_size(border_width: int, _border_padding: int) -> Optional[WindowsErrorMessage]:
    metrics = shell__get_raw_window_metrics()
    if isinstance(metrics, WindowsErrorMessage):
        return metrics
    metrics.iBorderWidth = DWORD(border_width)
    return inner__set_window_metrics(metrics)


def shell__open_start_menu(_: bool) -> Optional[WindowsErrorMessage]:
